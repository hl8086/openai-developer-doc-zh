...
Regional processing (data residency) endpoints are charged a 10% uplift for models released on or after March 5, 2026, that are eligible for data residency. See our [Your data](/api/docs/guides/your-data) guide for supported regions and processing details.
...
For image generation cost estimates, use the [calculator](/api/docs/guides/image-generation#calculating-costs) in the image generation guide.
...
All fine-tuned models will remain available for inference until their base models are deprecated. The full timeline is [here](/api/docs/deprecations#update-to-openais-self-serve-fine-tuning).
