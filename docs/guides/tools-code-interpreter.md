...
*   Boosting visual intelligence in our latest reasoning models (like [o3](/api/docs/models/o3) and [o4-mini](/api/docs/models/o4-mini)). The model can use this tool to crop, zoom, rotate, and otherwise process and transform images.

Here’s an example of calling the [Responses API](/api/docs/api-reference/responses) with a tool call to Code Interpreter:
...
The Code Interpreter tool requires a [container object](/api/docs/api-reference/containers/object). A container is a fully sandboxed virtual machine that the model can run Python code in. This container can contain files that you upload, or that it generates.
...
2.  Explicit mode: here, you explicitly [create a container](/api/docs/api-reference/containers/createContainers) using the `v1/containers` endpoint, including the `memory_limit` you need (for example `"memory_limit": "4g"`), and assign its `id` as the `container` value in the tool configuration in the Response object. For example:
...
You can download these constructed files by calling the [get container file content](/api/docs/api-reference/container-files/retrieveContainerFileContent) method.

Any [files in the model input](/api/docs/guides/file-inputs) get automatically uploaded to the container. You do not have to explicitly upload it to the container.

### Uploading and downloading files

Add new files to your container using [Create container file](/api/docs/api-reference/container-files/createContainerFile). This endpoint accepts either a multipart upload or a JSON body with a `file_id`. List existing container files with [List container files](/api/docs/api-reference/container-files/listContainerFiles) and download bytes from [Retrieve container file content](/api/docs/api-reference/container-files/retrieveContainerFileContent).
...
| --- | --- | --- |
| [Responses](/api/docs/api-reference/responses)[Chat Completions](/api/docs/api-reference/chat)[Assistants](/api/docs/api-reference/assistants) | 100 RPM per org | [Pricing](/api/docs/pricing#built-in-tools)  
