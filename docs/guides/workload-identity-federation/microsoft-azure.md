...
The Application ID URI configured in Microsoft Entra ID, the IMDS `resource` parameter, the resulting token’s `aud` claim, and the OpenAI Workload Identity Provider audience must all match.
