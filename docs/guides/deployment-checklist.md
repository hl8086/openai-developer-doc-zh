...
**Always start** with the [Responses API](/api/docs/guides/migrate-to-responses). It is OpenAI’s flagship API and the best place to access the newest model behavior, built-in tools, stateful workflows, and agent features.
...
[Built-in tools](/api/docs/guides/tools) are the API’s native capabilities. Instead of building every tool yourself, you can give the model access to tools that already work inside the Responses API. The model can then decide when to use them.
...
[Prompt caching](/api/docs/guides/prompt-caching) automatically reduces latency and cost when requests reuse the same long prefix. For high-volume workflows, set [`prompt_cache_key`](/api/docs/api-reference/responses/create#responses-create-prompt_cache_key) consistently for requests that share the same stable prefix.
...
Always round-trip reasoning items. This helps the model by allowing it to work from its prior reasoning. If your [Zero Data Retention (ZDR)](/api/docs/guides/your-data#zero-data-retention) requirements do not allow storing response data, this is where `reasoning.encrypted_content` is important. `reasoning.encrypted_content` gives you a stateless handoff.
...
Note: `background=True` is not compatible with [Zero Data Retention](/api/docs/guides/your-data#zero-data-retention).

## Use WebSocket mode

[WebSocket mode](/api/docs/guides/websocket-mode) is built for long-running, tool-call-heavy workflows where you keep a persistent connection open and continue by sending only new input items plus `previous_response_id`. For rollouts with 20 or more tool calls, this approach is roughly 40% faster end-to-end.
