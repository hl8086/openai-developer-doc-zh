...
The Audio API provides a [`speech`](/api/docs/api-reference/audio/createSpeech) endpoint based on our [GPT-4o mini TTS (text-to-speech) model](/api/docs/models/gpt-4o-mini-tts). It comes with 11 built-in voices and can be used to:
...
1.  The [model](/api/docs/api-reference/audio/createSpeech#audio-createspeech-model) you’re using
2.  The [text](/api/docs/api-reference/audio/createSpeech#audio-createspeech-input) to be turned into audio
3.  The [voice](/api/docs/api-reference/audio/createSpeech#audio-createspeech-voice) that will speak the output
...
If you’re using the [Realtime API](/api/docs/guides/realtime), note that the set of available voices is slightly different—see the [realtime conversations guide](/api/docs/guides/realtime-conversations#voice-options) for current realtime voices.
...
Custom voices enable you to create a unique voice for your agent or application. These voices can be used for audio output with the [Text to Speech API](/api/docs/api-reference/audio/createSpeech), the [Realtime API](/api/docs/api-reference/realtime), or the [Chat Completions API with audio output](/api/docs/guides/audio).

To create a custom voice, you’ll provide a short sample audio reference that the model will seek to replicate.

Custom voices are limited to eligible customers. Contact our [sales team](https://openai.com/contact-sales/) to learn more. Once enabled for your organization, you’ll have access to the [Voices](https://platform.openai.com/audio/voices) tab under Audio.
...
Speech generation will work as usual. Simply specify the ID of the voice in the `voice` parameter when [creating speech](/api/docs/api-reference/audio/createSpeech), or when initiating a [realtime session](/api/docs/api-reference/realtime/create-call#realtime_create_call-session-audio-output-voice).
...
[Realtime and audio overview - Choose the right path for voice agents, translation, transcription, and speech generation.](/api/docs/guides/realtime)

[Audio and speech concepts - Review audio modalities, speech tasks, streaming, and request-based APIs.](/api/docs/guides/audio)