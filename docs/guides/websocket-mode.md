...
The standalone [`/responses/compact` endpoint](/api/docs/api-reference/responses/compact) returns a new compacted input window, not a response ID. After compaction, create a new response on your WebSocket connection using the compacted window as `input` (plus the next user/tool items).
...
*   [Streaming API responses](/api/docs/guides/streaming-responses)
*   [Responses streaming events reference](/api/docs/api-reference/responses-streaming)