...
The data should be formatted in JSONL format, with each line [representing an example](/api/docs/api-reference/fine-tuning/preference-input) in the following structure:
...
Uploading training data and using a model fine-tuned with DPO follows the [same flow described here](/api/docs/guides/model-optimization).

To create a DPO fine-tune job, use the `method` field in the [fine-tuning job creation endpoint](/api/docs/api-reference/fine-tuning/create), where you can specify `type` as well as any associated `hyperparameters`. For DPO:
