...
[Voice agentsBuild speech-to-speech agents that listen, reason, speak, and call tools.](/api/docs/guides/voice-agents)

[Live translationTranslate live speech with a dedicated realtime translation session.](/api/docs/guides/realtime-translation)

[TranscriptionStream live transcript deltas or process audio files into text.](/api/docs/guides/realtime-transcription)

[Speech generationTurn text into natural-sounding spoken audio.](/api/docs/guides/text-to-speech)
...
Realtime 2 adds reasoning to speech-to-speech workflows. Start with `reasoning.effort` set to `low` for most production voice agents, then adjust based on latency tolerance and task complexity. Use the [Realtime prompting guide](/api/docs/guides/realtime-models-prompting) to tune reasoning, preambles, tool use, unclear audio, and exact entity capture.
...
[WebRTC - Use for browser and mobile clients that capture or play audio directly.](/api/docs/guides/realtime-webrtc)

[WebSocket - Use when your server already receives raw audio from a media pipeline, call system, or worker.](/api/docs/guides/realtime-websocket)

[SIP - Use for telephony voice agents. Confirm model support before using SIP for translation or transcription.](/api/docs/guides/realtime-sip)
...
Use [Audio and speech](/api/docs/guides/audio) for the core concepts behind audio input, audio output, streaming, latency, transcripts, and speech generation. Use this overview when you are ready to choose an implementation path.