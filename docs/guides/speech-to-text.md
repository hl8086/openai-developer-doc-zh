...
Use this guide for file uploads and bounded audio requests. If your application needs live transcript deltas from a microphone, call, or media stream, use [Realtime transcription](/api/docs/guides/realtime-transcription) instead.
...
The [API Reference](/api/docs/api-reference/audio) includes the full list of available parameters.
...
You can use a [prompt](/api/docs/api-reference/audio/createTranscription#audio/createTranscription-prompt) to improve the quality of the transcripts generated by the Transcriptions API.
...
For live audio from a microphone, call, or media stream, use the [Realtime transcription](/api/docs/guides/realtime-transcription) guide instead of the file-oriented streaming path above. It covers the current transcription-session flow and the recommended realtime path with [`gpt-realtime-whisper`](/api/docs/models/gpt-realtime-whisper).
