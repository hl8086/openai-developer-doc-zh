...
*   Start with the [MCP and Connectors guide](/api/docs/guides/tools-connectors-mcp) for general MCP concepts.
...
The private MCP server does not need a public listener. The OpenAI-hosted endpoint gives supported products a normal MCP request path, while the network initiation point stays inside your boundary. When a connector asks for streamed results, the tunnel path can forward intermediate server-sent events.
 ...
 ![Diagram showing an OpenAI product sending MCP JSON-RPC through the OpenAI tunnel service to tunnel-client, which forwards the request to a private MCP server and returns the response through the same tunnel.](/images/platform/guides/secure-mcp-tunnels/request-flow-diagram.png) ![Diagram showing an OpenAI product sending MCP JSON-RPC through the OpenAI tunnel service to tunnel-client, which forwards the request to a private MCP server and returns the response through the same tunnel.](/images/platform/guides/secure-mcp-tunnels/request-flow-diagram.png)
...
Keep `tunnel-client run ...` healthy while you create or test the connector. Connector discovery and MCP tool calls depend on the running client.
 ...
 ![Live local tunnel-client admin UI showing health, readiness, tunnel metadata, and channel status.](/images/platform/guides/secure-mcp-tunnels/tunnel-client-admin-ui.png) ![Live local tunnel-client admin UI showing health, readiness, tunnel metadata, and channel status.](/images/platform/guides/secure-mcp-tunnels/tunnel-client-admin-ui.png)
...
## Security and networking
 ...
 ![Diagram showing tunnel-client inside the customer-controlled environment connecting outbound to the OpenAI-managed tunnel control plane while the private MCP server remains inside the customer network.](/images/platform/guides/secure-mcp-tunnels/trust-boundaries-diagram.png) ![Diagram showing tunnel-client inside the customer-controlled environment connecting outbound to the OpenAI-managed tunnel control plane while the private MCP server remains inside the customer network.](/images/platform/guides/secure-mcp-tunnels/trust-boundaries-diagram.png)
...
[![Sanitized OpenAI Platform tunnel settings screenshot.](/images/platform/guides/secure-mcp-tunnels/platform-tunnels-settings.png)](https://platform.openai.com/settings/organization/tunnels)

Create and manage OpenAI-hosted MCP tunnel endpoints from Platform tunnel settings.

[![Sanitized ChatGPT connector settings screenshot with Tunnel selected.](/images/platform/guides/secure-mcp-tunnels/chatgpt-connectors-tunnel.png)](https://chatgpt.com/#settings/Connectors)
