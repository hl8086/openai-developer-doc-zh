...
With the introduction of [GPT-5](/api/docs/models/gpt-5), we added some checks to find and halt hazardous information from being accessed. It’s likely some users will eventually try to use your application for things outside of OpenAI’s policies, especially in applications with a wide range of use cases.
...
*   [Implement safety identifiers](/api/docs/guides/safety-best-practices#implement-safety-identifiers) for products where individual users interact with a model. Safety identifiers are recommended but not required.
...
The `safety_identifier` parameter is available in both the [Responses API](/api/docs/api-reference/responses/create) and older [Chat Completions API](/api/docs/api-reference/chat/create). The Realtime API supports the same concept through the `OpenAI-Safety-Identifier` header. To use safety identifiers, provide a stable ID for your end user on each request. Hash user email or internal user IDs to avoid passing any personal information.
...
*   [Fine-tuning safety](/api/docs/guides/supervised-fine-tuning#safety-checks)
*   [Safety checks in computer use](/api/docs/guides/tools-computer-use#acknowledge-safety-checks)