...
You can use `file_id` (from the [Files API](/api/docs/api-reference/files)) or `image_url` (a URL or base64 data URL). See [images and vision](/api/docs/guides/images-vision) for details.
...
[File inputs](/api/docs/guides/pdf-files)—currently PDFs—are supported. Pass `file_id`, `file_url`, or `file_data` as you would for `responses.create`. The token count reflects the model’s full processed input.
