...
Here’s a simple example using the [Chat Completions API](/api/docs/api-reference/chat).
...
When in doubt, [`gpt-5.5`](/api/docs/models/gpt-5.5) offers a strong default for general-purpose text generation and prompt iteration.
...
Reusable prompts are currently supported in the [Responses API](/api/docs/guides/text?api-mode=responses#reusable-prompts) only. They are not available in the Chat Completions API.
...
GPT models like [`gpt-5.5`](/api/docs/models/gpt-5.5) benefit from precise instructions that explicitly provide the logic and data required to complete the task in the prompt. To get the most out of the latest GPT-5 series model, start with the current prompting guide.

[GPT-5.5 prompting guide - Get the most out of prompting the latest GPT-5 series model with current guidance, practical examples, and migration notes.](/api/docs/guides/prompt-guidance)

### Prompting best practices for the latest GPT-5 series model

For the full current treatment, use the [prompt guidance](/api/docs/guides/prompt-guidance) guide. The practical reminders below still apply.
...
For detailed guidance and prompt samples specific to coding, see our [prompt guidance](/api/docs/guides/prompt-guidance) guide.

Front-end engineering

[GPT-5.5](/api/docs/models/gpt-5.5)

performs well at building front ends from scratch as well as contributing to large, established codebases. To get the best results, we recommend using the following libraries:
...
For detailed guidance and prompt samples specific to frontend development, see our [prompt guidance](/api/docs/guides/prompt-guidance) guide.
...
For detailed guidance and prompt samples specific to building agents, see the [prompt guidance](/api/docs/guides/prompt-guidance) guide.
...
[Generate JSON data with Structured Outputs - Ensure JSON data emitted from a model conforms to a JSON schema.](/api/docs/guides/structured-outputs)

[Full API reference - Check out all the options for text generation in the API reference.](/api/docs/api-reference/responses)
