...
If your workflow only needs a short model response and no persistent workspace, call the [Responses API](/api/reference/responses/overview) directly or use the basic Agents SDK runtime without a sandbox.

If shell access is only one occasional tool, start with the hosted shell tool in [Using tools](/api/docs/guides/tools#usage-in-the-agents-sdk). Use sandbox agents when workspace isolation, sandbox provider choice, or resumable filesystem state is part of the product design.

## What sandboxes add

`SandboxAgent` is still an `Agent`. It keeps the usual agent surface, including `instructions`, `prompt`, `tools`, `handoffs`, MCP servers, model settings, structured output, guardrails, and hooks. What changes is the execution boundary: the runner prepares the agent against a live sandbox session that owns files, commands, ports, and provider-specific isolation.
...
Sandbox-specific defaults belong on `SandboxAgent`. Per-run sandbox-session choices belong in the run’s sandbox configuration.
...
`Manifest` describes the desired starting contents and layout for a fresh sandbox workspace. Use it for the files, repos, input artifacts, helper files, mounts, output directories, and environment setup the agent should see.
...
Manifest entry paths are workspace-relative. They can’t be absolute paths or escape the workspace with `..`, which keeps the workspace contract portable across local, Docker, and hosted clients.
...
By default, a `SandboxAgent` includes filesystem, shell, and compaction capabilities. If you pass a `capabilities` list, it replaces the default list, so include any default capabilities the agent still needs.
...
Some tasks need repeatable instructions, scripts, references, or assets before the agent starts. Use the `Skills` capability so the agent can discover that working context during the run.
...
The provider is part of the run configuration, not the agent definition. Keep the `SandboxAgent`, manifest, and capabilities stable, then swap the sandbox client and provider options for the environment you want.
...
Fresh-session inputs such as `manifest` and `snapshot` only apply when the runner creates a new sandbox session. If you inject a live `session`, capability processing can add compatible non-mount entries, but it can’t change root, environment, users, or groups; remove existing entries; replace entry types; or add or change mount entries on the already-running sandbox.

This split lets the harness resume the agent loop while the sandbox provider restores or recreates the workspace. Current sample code for these paths lives in the TypeScript [resume session state example](https://github.com/openai/openai-agents-js/blob/main/examples/docs/sandbox-agents/resume-session-state.ts) and Python [`main.py`](https://github.com/openai/openai-agents-python/tree/main/examples/sandbox/tutorials/sandbox_resume) and [`sandbox_agent_with_remote_snapshot.py`](https://github.com/openai/openai-agents-python/blob/main/examples/sandbox/sandbox_agent_with_remote_snapshot.py).

## Persist memory across runs

Sandbox memory lets future sandbox-agent runs learn from prior runs. It’s separate from SDK-managed conversational `Session` memory: sessions preserve message history, while sandbox memory distills useful lessons from prior workspace runs into files the agent can read later.
...
Memory reads use progressive disclosure. The SDK injects `memory_summary.md` at the start of a run, the agent searches `MEMORY.md` when prior work looks relevant, and it opens rollout summaries only when it needs more detail.
...
For runnable examples, see the TypeScript [memory guide](https://openai.github.io/openai-agents-js/guides/sandbox-agents/memory), plus Python [`memory.py`](https://github.com/openai/openai-agents-python/blob/main/examples/sandbox/memory.py) for a local snapshot flow, [`memory_s3.py`](https://github.com/openai/openai-agents-python/blob/main/examples/sandbox/memory_s3.py) for S3-backed memory storage, and [`memory_multi_agent_multiturn.py`](https://github.com/openai/openai-agents-python/blob/main/examples/sandbox/memory_multi_agent_multiturn.py) for separate memory layouts across agents.
...
For examples, see [`handoffs.py`](https://github.com/openai/openai-agents-python/blob/main/examples/sandbox/handoffs.py) and [`sandbox_agents_as_tools.py`](https://github.com/openai/openai-agents-python/blob/main/examples/sandbox/sandbox_agents_as_tools.py).
