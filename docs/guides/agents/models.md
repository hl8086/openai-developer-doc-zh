...
For most new SDK workflows, start with [`gpt-5.5`](/api/docs/models/gpt-5.5) and move to a smaller variant only when latency or cost matters enough to justify it. Use the platform-wide [Using GPT-5.5](/api/docs/guides/latest-model) guide for current model-selection advice.
...
*   Live audio sessions over WebRTC or WebSocket are for low-latency voice or image interactions. Use [Voice agents](/api/docs/guides/voice-agents) and the [live audio API guide](/api/docs/guides/realtime) for that path.
...
[Agent definitions - Keep model choices aligned with the responsibilities of each specialist.](/api/docs/guides/agents/define-agents)

[Running agents - See how transport and model choices affect the runtime loop.](/api/docs/guides/agents/running-agents)

[External models - Compare broader provider options when a mixed-model stack matters.](/api/docs/guides/external-models)