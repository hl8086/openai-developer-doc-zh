...
[Set up evals →](/api/docs/guides/evals)
...
*   We recommend starting with 50 well-crafted demonstrations and [evaluating the results](/api/docs/guides/evals)
...
*   Use historical data, expert data, logged data, or [other types of collected data](/api/docs/guides/evals)

### Formatting your data

*   Use [JSONL format](https://jsonlines.org/), with one complete JSON structure on every line of the training data file
*   Use the [chat completions format](/api/docs/api-reference/fine-tuning/chat-input)
...
*   Capture results generated from your model using whatever technique is convenient - note that the [Responses API](/api/docs/api-reference/responses) stores model responses for 30 days by default.
...
In addition to text completions, you can train the model to more effectively generate [structured JSON output](/api/docs/guides/structured-outputs) or [function calls](/api/docs/guides/function-calling).
...
With your test data uploaded, [create a fine-tuning job](/api/docs/api-reference/fine-tuning/create) to customize a base model using the training data you provide. When creating a fine-tuning job, you must specify:

*   A base model (`model`) to use for fine-tuning. This can be either an OpenAI model ID or the ID of a previously fine-tuned model. See which models support fine-tuning in the [model docs](/api/docs/models).
...
Create a supervised fine-tuning job by calling the [fine-tuning API](/api/docs/api-reference/fine-tuning):
...
The API responds with information about the fine-tuning job in progress. Depending on the size of your training data, the training process may take several minutes or hours. You can [poll the API](/api/docs/api-reference/fine-tuning/retrieve) for updates on a specific job.
...
Note the `fine_tuned_model` property. This is the model ID to use in [Responses](/api/docs/api-reference/responses) or [Chat Completions](/api/docs/api-reference/chat) to make API requests using your fine-tuned model.
...
[Reinforcement fine-tuning - Fine-tune a reasoning model by grading its outputs.](/api/docs/guides/reinforcement-fine-tuning)