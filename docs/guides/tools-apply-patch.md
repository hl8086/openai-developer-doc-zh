...
Alternatively, you can use the [Agents SDK](/api/docs/guides/tools#usage-in-the-agents-sdk) to use the apply patch tool. You’ll still have to implement the harness that handles the actual file operations but you can use the `applyDiff` function to handle the diff processing.
...
[Apply patch tool example - TypeScript - Example of how to use the apply patch tool with the Agents SDK in TypeScript](https://github.com/openai/openai-agents-js/blob/main/examples/tools/applyPatch.ts)

[Apply patch tool example - Python - Example of how to use the apply patch tool with the Agents SDK in Python](https://github.com/openai/openai-agents-python/blob/main/examples/tools/apply_patch.py)
