...
When we create the run, we set up a prompt using either a [Chat Completions](/api/docs/guides/text?api-mode=chat) messages array or a [Responses](/api/docs/api-reference/responses) input. This prompt is used to generate a model response for every line of test data in your data set. We can use the double curly brace syntax to template in the dynamic variable `item.ticket_text`, which is drawn from the current test data item.
...
[Model distillation - Learn how to distill large model results to smaller, cheaper, and faster models.](/api/docs/guides/distillation)