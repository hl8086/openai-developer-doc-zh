...
To learn more about image generation, refer to our dedicated [image generation guide](/api/docs/guides/image-generation?api=responses).
...
You can [provide input images](/api/docs/guides/image-generation?image-generation-model=gpt-image#edit-images) using file IDs or base64 data.

To force the image generation tool call, you can set the parameter `tool_choice` to `{"type": "image_generation"}`.

### Tool options

You can configure the following output options as parameters for the [image generation tool](/api/docs/api-reference/responses/create#responses-create-tools):
...
`gpt-image-2` supports flexible `size` values that meet its [resolution constraints](/api/docs/guides/image-generation#size-and-quality-options). It doesn’t currently support transparent backgrounds, so requests with `background: "transparent"` fail.

For more details on available options, refer to the [image generation guide](/api/docs/guides/image-generation#customize-image-output).
