...
[Reinforcement fine-tuning](/api/docs/guides/reinforcement-fine-tuning) (RFT) provides a way to improve your model’s performance at specific tasks. The task must be clear and have verifiable answers.
...
[Ready to use reinforcement fine-tuning? Skip to the guide →](/api/docs/guides/reinforcement-fine-tuning)
...
An effective eval reveals opportunities where human experts consistently agree but current frontier models struggle, presenting a valuable gap for RFT to close. [Get started with evals](/api/docs/guides/evals).
...
*   **Make sure each answer can be graded**. A grader must read an answer and produce a score without a person in the loop. We support multiple [grader types](/api/docs/guides/graders), including custom Python graders and LLM judges. If you can’t write code to judge the answer with an available grader, RFT is not the right tool.
...
*   **Use an LLM judge when code falls short**. For rich, open‑ended answers, have a [separate OpenAI model grade](/api/docs/guides/graders#model-graders) your fine-tuned model’s answers. Make sure you:
...
Learn more about [grader types](/api/docs/guides/graders).
...
*   [Reinforcement fine-tuning guide](/api/docs/guides/reinforcement-fine-tuning)
*   [Graders](/api/docs/guides/graders)
*   [Model optimization overview](/api/docs/guides/model-optimization)