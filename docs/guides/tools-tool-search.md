...
2.  If you are using [functions](/api/docs/guides/function-calling#defining-functions), mark the ones you want to defer with `defer_loading: true`. If you are using [MCP servers](/api/docs/guides/tools-connectors-mcp), set `defer_loading: true` on the MCP server tool definition.
...
*   Use [function calling](/api/docs/guides/function-calling) to define callable functions and custom tools.
*   Use [Using tools](/api/docs/guides/tools) for the broader tool landscape across Responses.