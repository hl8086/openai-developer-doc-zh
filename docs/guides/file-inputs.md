...
*   Use [File Search](/api/docs/guides/tools-file-search) for retrieval over large files instead of passing them directly as `input_file`.
*   Use [Hosted Shell](/api/docs/guides/tools-shell#hosted-shell-quickstart) for spreadsheet-heavy tasks that need detailed analysis, such as aggregations, joins, charting, or custom calculations.
...
To preserve chart and diagram fidelity, convert the file to PDF first, then send the PDF as `input_file`.

## How spreadsheet augmentation works

For spreadsheet-like files (such as `.xlsx`, `.xls`, `.csv`, `.tsv`, and `.iif`), `input_file` uses a spreadsheet-specific augmentation process.
...
The following table lists common file types accepted in `input_file`. The full list of extensions and MIME types appears later on this page.
...
Chat Completions does not support file URLs. Use the [Responses API](/api/docs/guides/file-inputs?api-mode=responses#file-urls) for this option.

## Uploading files

The following example uploads a file with the [Files API](/api/docs/api-reference/files), then references its file ID in a request to the model.
...
*   **Token usage:** PDF parsing includes both extracted text and page images in context, which can increase token usage. Before deploying at scale, review pricing and token implications. [More on pricing](/api/docs/pricing).
...
*   **File upload purpose:** You can upload files with any supported [purpose](/api/docs/api-reference/files/create#files-create-purpose), but use `user_data` for files you plan to pass as model inputs.
...
[Experiment with file inputs in the Playground - Use the Playground to develop and iterate on prompts with file inputs.](https://platform.openai.com/chat/edit)

[Full API reference - Check out the API reference for more options.](/api/docs/api-reference/responses)

[Use File Search for large corpora - Use retrieval over chunked files when you need scalable search instead of sending whole files in a single context window.](/api/docs/guides/tools-file-search)

[Use Hosted Shell for deep spreadsheet analysis - Use Hosted Shell for advanced spreadsheet workflows such as joins, aggregations, and charting.](/api/docs/guides/tools-shell#hosted-shell-quickstart)