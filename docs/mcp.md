...
In this guide, we’ll cover how to build a remote MCP server that reads data from a private data source (a [vector store](/api/docs/guides/retrieval)) and makes it available in ChatGPT as a data-only app (formerly called a connector) for chat, deep research, and company knowledge, as well as [via API](/api/docs/guides/deep-research).

**Note**: For ChatGPT app setup (developer mode, connecting your MCP server, and optional UI), start with the Apps SDK docs: [Quickstart](/apps-sdk/quickstart), [Build your MCP server](/apps-sdk/build/mcp-server), [Connect from ChatGPT](/apps-sdk/deploy/connect-chatgpt), and [Authentication](/apps-sdk/build/auth). If you are building a data-only app, you can skip UI resources and just expose tools.
...
You can use data from any source to power a remote MCP server, but for simplicity, we will use [vector stores](/api/docs/guides/retrieval) in the OpenAI API. Begin by uploading a PDF document to a new vector store - [you can use this public domain 19th century book about cats](https://cdn.openai.com/API/docs/cats.pdf) for an example.

You can upload files and create a vector store [in the dashboard here](https://platform.openai.com/storage/vector_stores), or you can create vector stores and upload files via API. [Follow the vector store guide](/api/docs/guides/retrieval) to set up a vector store and upload a file to it.
...
To work with ChatGPT deep research and company knowledge (and deep research via API), your MCP server should implement two read-only tools: `search` and `fetch`, using the compatibility schema in [Company knowledge compatibility](/apps-sdk/build/mcp-server#company-knowledge-compatibility).

Declare an output schema for each tool so clients can validate the result shape. In FastMCP, typed return models can generate this schema automatically; the example below passes `output_schema` explicitly from the same models.
...
In MCP, return this object as `structuredContent` and include the same value as a JSON-encoded string in the [content array](https://modelcontextprotocol.io/docs/learn/architecture#understanding-the-tool-execution-response) for compatibility.
...
In MCP, return this object as `structuredContent` and include the same value as a JSON-encoded string in the content array for compatibility.
...
If you are testing this server in ChatGPT as an app, follow [Connect from ChatGPT](/apps-sdk/deploy/connect-chatgpt).
...
2.  Create and configure your app in **Apps & Connectors** using your server URL.
3.  Test your app by running prompts in chat and deep research.

For detailed setup steps, see [Connect from ChatGPT](/apps-sdk/deploy/connect-chatgpt).
