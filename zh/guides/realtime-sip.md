...
> 通过 SIP 协议将电话系统连接到 Realtime API。
...
通过此 webhook，你可以使用 webhook 中的 `call_id` 值来接受或拒绝通话。接受通话时，你需要为 Realtime API 会话提供所需的配置（指令、语音等）。建立连接后，你可以像往常一样设置 WebSocket 并监控会话。接受、拒绝、监控、转接和挂断通话的 API 文档如下。
...
如果未提供状态码，API 默认使用 `603 Decline`。成功的请求在 OpenAI 发送 SIP 响应后返回 `200 OK`。

## 监控通话事件

接受通话后，打开一个 WebSocket 连接到同一会话以流式传输事件并发出实时命令。请注意，当使用 `call_id` 参数连接到现有通话时，`model` 参数不会被使用（因为它已经通过 `accept` 端点配置好了）。
...
OpenAI 在 REFER 被转发到你的 SIP 提供商后返回 `200 OK`。下游系统处理呼叫者的后续通话流程。
...
如果你需要将 OpenAI SIP 流量加入白名单。`sip.api.openai.com` 使用 GeoIP 路由，你将连接到最近的区域。

*   `13.79.45.80/28` 用于 `northeurope`
*   `23.98.140.64/28` 用于 `southcentralus`
*   `40.67.149.176/28` 用于 `eastus2`
*   `40.83.204.240/28` 用于 `westus`

## Python 示例

以下是一个 `realtime.call.incoming` 处理程序的示例。它接受通话，然后记录来自 Realtime API 的所有事件。
