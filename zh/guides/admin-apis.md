...
> Admin APIs 允许你自动化组织管理工作流，例如用户邀请、审计日志审查、项目管理、API 密钥管理、支出警报、数据保留和速率限制操作。
...
要访问这些端点，请[创建一个 Admin API 密钥](https://platform.openai.com/settings/organization/admin-keys)。Admin API 密钥不能用于非管理端点。

以下 SDK 版本中添加了对 Admin APIs 的支持，你可能需要更新 SDK 版本：
...
使用项目支出警报在项目支出达到阈值时通知你的团队。阈值金额以美分为单位指定。
